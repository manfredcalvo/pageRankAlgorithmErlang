...
readInputMatrixFile(Device, S, N, K, Workers) ->
    case io:get_line(Device, "") of
        ...
        Line -> Neighboors = stringToAtom(Line),
...
visitNeighboors([A|T], S, L, N, K, Workers) ->
...
separateVectorOnBlocks(_, N, K, 0) -> separateOnBlocksFirstTime(1, N, K);
separateVectorOnBlocks(FileName, _, K, _) -> {ok, Device} = file:open(FileName, [read]),
...
separateOnBlocksFirstTime(I, N, _) when I > N -> ok;
separateOnBlocksFirstTime(I, N, K) -> writeVectorPairOnAFile(((I - 1) div K) + 1, {I, 1 / N}), separateOnBlocksFirstTime(1 + I, N, K).
...
pageRank(MatrixFileName, N, K, Iterations, Beta, Nodes) ->	
								...
								pageRankAux("", N, K, Iterations, 0, MapSlaves, ReduceSlaves),
...
pageRankAux(VectorFileName, N, K, Iterations, Actual, MapSlaves, ReduceSlaves) -> 
						 ...
						 separateVectorOnBlocks(VectorFileName, N, K, Actual),
...
processVectorEntries(N, Limit, Parameters, Master) -> 
						Value = readVectorFileReduce(Parameters, file:open(getVectorPosFileName(N), [read])), 
						Master ! {N, Value}, processVectorEntries(N + 1, Limit, Parameters, Master). 


getVectorPosFileName(Pos) -> lists:concat([?FOLDER ++ "VectorPos/Vector", Pos, "Pos.csv"]).


readVectorFileReduce(_, {error,enoent}) -> 0 ; 
readVectorFileReduce(Parameters, {ok, Device}) -> readVectorFileReduceAux(Device, Parameters, 0).
